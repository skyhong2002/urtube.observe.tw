...
candidate card. Accepting unlocks only each person's self-authored introduction
and contact method. Decline, withdrawal, opt-out, and account deletion revoke
the server-side relationship. Every contact read verifies the accepted request
and current opt-in instead of relying on hidden browser markup.
