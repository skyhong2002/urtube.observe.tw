...
const viewerCrystal = () => crystal(
  [topic('music', 1)],
  [{ key: 'anchor', name: 'Anchor Channel', share: 1 }],
);

const neighborCrystal = () => crystal(
  [topic('music', 0.6), topic('gaming', 0.4)],
  [
    { key: 'anchor', name: 'Anchor Channel', share: 0.6 },
    { key: 'discovery', name: 'Discovery Channel', share: 0.4 },
  ],
);
...
test('cohort recommendations honor exclusions, disclosure, neighbor bounds, and output caps', () => {
  ...
  const manyItems = crystal(
    ...
    [
      { key: 'anchor', name: 'Anchor Channel', share: 0.4 },
  ...
  const capped = cohortRecommendations(
    ...
    {
      blockedChannelKeys: new Set(['new-0']),
      viewerSeenChannelKeys: new Set(['new-1']),
    },
  );
...
test('/matches removes governed channels and recomputes an anonymous cohort after opt-out', async () => {
  ...
  const app = createApp(registry, {
    loadTagLists: async () => emptyTagLists(['blocked-politics']),
  });
  try {
    ...
    const neighbors = [1, 2, 3].map((index) => {
      ...
      value.channels = [
        { key: 'anchor', name: 'Anchor Channel', share: 0.4 },
        { key: 'discovery', name: 'Discovery Channel', share: 0.25 },
        { key: 'blocked-politics', name: 'Blocked Politics Channel', share: 0.2 },
