...
本文依上述程式基準，說明架構、資料流程、AI 使用方式、核心數學模型、可靠性、授權、負載分析與可重現驗證。容量分析採用 100 人、每人 20,000 部影片的情境，並區分實作行為、隔離測試結果與待驗證的容量假設。
...
驗證範圍涵蓋資料契約、計算性質、失敗處理與授權邊界。分類品質與門檻校準尚缺完整的人工標註語意相似度資料集，真人配對成效亦尚未經使用者研究驗證。[1][2]
...
實作以 SciPy linprog 與 HiGHS 最小化運輸成本 1－k，與上式等價。最多十群對十群，因此最多 100 個變數、20 列等式約束。HiGHS 自動選擇求解方法，求解成本取決於所選方法及問題結構。[8][9]
...
正式入口為 /matches。啟用 v3 時，同頁保留具名成員、頭像、Profile、好友邀請與 Blend；沒有可計算分數時以「—」表示。/matching-v3 轉向整合入口的主題檢視。[2]
...
每帳號工作保存 fingerprint、演算法版本、狀態、租約 token、租約期限、重試次數與時間。原子 UPDATE RETURNING 領取 queued 或過期的 running 工作，取得 180 秒租約；處理期間更新心跳。租約支援中斷後重新領取工作；外部 API 請求可能因失敗重試而重送。[11]
...
觀看時間推估的退化項為各影片未量測事件數與同片事件數乘積的總和。令 nᵥ 為同片觀看數；基準中每部只看一次，Σnᵥ²＝20,000。若事件集中在少數影片，重看次數的平方項才會顯著增加，因此容量估算需要同時考慮事件總量與重看分布。
...
2026 年 9 月 6 日，於本文程式基準以 lockfile 安裝依賴後執行 npm run check：TypeScript 檢查通過；Node 測試共 307 項，306 通過、0 失敗、1 跳過。跳過項為需另啟數值服務的 Node 至 Python HTTP 整合測試；該項在隔離 localhost 服務上獨立執行並通過。這組測試以合成向量驗證跨服務計算契約，驗證範圍未涵蓋正式環境負載。[14]

Python 數值測試使用隔離環境與 requirements.txt 的固定版本，結果見本節驗證表。所有 fixture 使用合成身分與固定向量，未讀取正式使用者資料，未呼叫付費分類或 embedding API。

| 驗證範圍 | 核心性質與案例 | 證據定位 |
| --- | --- | --- |
| TypeScript 與 Node 測試 | 匯入、聚合、配對、授權、失敗與監控 | Node 306 項通過；HTTP 整合項獨立執行通過 |
...
本文程式基準的 GitHub Actions 工作流使用 Node 22，依序執行 npm ci、npm run check 與 production image build。Python 六項數值測試另行執行，尚未納入該工作流；第 11.1 節記錄的是隔離環境中的測試結果。[14]
...
容量驗收規劃使用隔離的百帳號資料，分別測試 1、10、100 個同時互動者，涵蓋冷／暖快取、匯入、背景分群及跨日刷新，量測 CPU、RSS、SQL 鎖等待、RPC 排隊與 HTTP 尾延遲。語意品質驗證需要人工標註與盲測，以校準 eps、similarity floor 及覆蓋門檻；現有門檻仍屬初始設定。
...
| 分布解釋 | 計算 47.37% 固定向量案例及共同群貢獻 | 加權 DBSCAN 與最佳傳輸 |
| 授權變更 | 發出／接受好友邀請，再撤回關係或類別 | server-side 授權與非同步重驗 |
| 恢復與維運 | 以合成測試重現部分成功、重試與過期租約 | 成功快取、token、原子發布 |

npm run demo:matching 提供可重設的 Alice／Bob 合成社交流程。v3 語意計算範例另需已完成的合成 profile 或測試 provider。雙帳號範例用於重現功能操作；兩百萬筆資料的容量分析見第 10 節。
...
容量擴展需維持分數語意、逐次授權與資料完整性。上述優化以減少重複讀取、分群與比較為核心，並透過第 11.3 節的容量與品質驗收確認成效。

## 13 實作證據與參考資料

以下程式與測試均以本文件首頁列出的 commit 為基準，供查核數學模型、服務契約及版本行為。第 [13] 項另提供對應的負載分析、合成測試程式與原始量測資料。
...
本文件使用的 DBSCAN 與 SciPy 文件版本對應服務鎖定依賴；外部文件查閱日為 2026 年 9 月 6 日。模型名稱對應本文程式基準的設定；供應商配額、價格與正式環境吞吐不在本次量測範圍內。
