...
`MATCHING_V3_ADMIN_HANDLES` 為以逗號分隔的帳號白名單，預設空白（無人可讀）。使用既有 Google 登入 session；一般登入使用者也無法讀取管理 API。管理員在配對頁可見「資料處理監控」，入口 `/matching-v3/admin`，每 5 秒更新。提供共用分類／向量快取、各帳號工作進度、九類輪廓狀態、worker 心跳、近五分鐘操作數與最近 50 次操作。操作記錄只保存時間、類型、項目數及安全錯誤碼，保留最近約 2,000 筆；不保存提示詞或原始回應。監控啟用前的呼叫數不回填捏造。
...
依使用者要求，候選須同時開啟既有 dashboardPublic（公開 profile）及 matchingOptIn，
並同意所有選定類別。計算前、計算後及回傳前檢查；API 回傳顯示名稱、handle 與 HTTPS
頭像 URL，不回傳 email 或密鑰。前端以文字節點顯示名稱，無頭像／載入失敗用名稱首字。
