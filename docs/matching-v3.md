...
`GET /api/processing` 僅回傳登入者自己的六項進度：影片資料、主題動態分類、常見關鍵字來源、v3 分類、目前批次向量與頻道階段。無資料、停用、等待、失敗及完成仍顯示進度列。關鍵字是在讀取儀表板時確定性計算，來源進度沿用 metadata，不建立額外 AI 工作。頻道尚無逐項計數時不偽造完成百分比；新入口不依賴 v3 啟用或配對同意。它只讀取既有結果，沿用簡潔模式與 30 秒輪詢。

剩餘時間取最近兩次以上的實際進展估算；換批次、來源總數改變、失敗或超過兩分鐘沒有進展會取消估算。沒有足夠樣本時顯示「估算中／尚無可估算的完成時間」，不以固定批次上限冒充處理速度。預估僅為該階段，不含排隊與供應商重試。
