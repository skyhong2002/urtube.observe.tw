...
export function memberProfilePage(viewerHandle: string, profile: MemberProfileData, lang: Lang): string {
  ...
  const actions = `${profile.friendship ? friendshipActions(profile.friendship, viewerHandle, t, `/${profile.handle}`, !profile.comparisonHref) : ''}${profile.comparisonHref ? `<a class="mp-blend" href="${html(profile.comparisonHref)}">${html(t.memberProfileBlend)}</a>` : ''}`;
  const body = `<style>
    .mp-profile .mt-actions{display:flex;flex-wrap:wrap;gap:10px;align-items:center;justify-content:center;margin:0}.mp-profile .mt-actions form{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin:0}.mp-profile .mt-actions button{border:0;border-radius:999px;cursor:pointer;font:inherit;font-size:15px;font-weight:700;min-height:48px;min-width:140px;padding:12px 24px}.mp-profile .mt-want{background:var(--accent);color:white}.mp-profile .mt-secondary{background:var(--raised);color:var(--ink-2)}.mp-profile .mt-state{color:var(--muted);font-size:13px}
    ...
    .mp-title-row{display:flex;align-items:center;justify-content:center;flex-wrap:wrap;gap:12px 20px}.mp-title-row .mp-blend{margin:0}
  </style><article class="mp-profile"><a class="mp-back" href="/matches?lang=${lang}">← ${html(t.navMatches)}</a><header>${avatar}<div class="mp-title-row"><h1>${html(profile.displayName)}</h1><div class="mt-actions">${actions}</div></div><p class="mp-handle">@${html(profile.handle)}</p></header>${profile.interests.length ? `<section class="mp-interests"><h2>${html(t.memberProfileInterests)}</h2><div class="mp-pills">${profile.interests.map(interest => `<span>${html(interest)}</span>`).join('')}</div></section>` : ''}<p class="mp-note">${html(t.memberProfilePrivate)}</p></article>`;
