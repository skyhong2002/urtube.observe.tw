...
const en = {
  ...
  privacySections: [
    ...
    ['Where it is stored', 'On this self-hosted server only. Each account has its own database file. Search terms are AES-256-GCM encrypted at rest; access tokens are stored as hashes only. Nothing is sent to any third party — no ads, no analytics, no selling of data.'],
...
const zh: typeof en = {
  ...
  privacySections: [
    ...
    ['存放在哪裡', '只存在這台自架伺服器上。每個帳號有獨立的資料庫檔案；搜尋詞以 AES-256-GCM 加密儲存；存取 token 只保存雜湊值。不會傳給任何第三方——沒有廣告、沒有分析追蹤、不出售資料。'],
