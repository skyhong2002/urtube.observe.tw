...
function monthLabel(month: string, t: Messages): string {
  ...
  return t.monthYear(year, number);
}
...
export function buildTopicTrendModel(months: YoutubeTopicTrendMonth[], t: Messages): TopicTrendModel {
  ...
  for (const month of months) {
    for (const topic of month.topics) {
      names.set(topic.slug, topic.name);
...
function raceRows(model: TopicTrendModel, index: number, t: Messages): string {
  ...
  return visible.map((value, rank) => {
    ...
    return `<li class="yt-trend-race-row" data-race-slug="${html(value.slug)}">
...
function heatmapRows(model: TopicTrendModel, t: Messages): string {
  return model.topics.map((topic) => {
    ...
    const cells = model.frames.map((frame) => {
      ...
      return `<button type="button" class="yt-trend-heat-cell${frame.provisional ? ' is-provisional' : ''}" style="--heat:${opacity.toFixed(3)}" aria-label="${html(details)}" title="${html(details)}">${percentage(value.share)}</button>`;
    }).join('');
    return `<div class="yt-trend-heat-row" data-topic-row data-latest="${stats.latest}" data-growth="${stats.growth}" data-average="${stats.average}" data-name="${html(topic.name)}">
...
export function topicTrendSection(data: YoutubeDashboardData, t: Messages): string {
  ...
  if (!model.topics.length || !model.frames.some((frame) => frame.values.some((value) => value.share !== null))) {
  ...
  const summary = t.topicTrendSummary(
    latest.label,
  ...
  const tableRows = model.frames.map((frame) => {
    const values = sortedValues(frame).filter((value) => value.estimatedWatchSeconds > 0)
      .map((value) => `${html(value.name)} · ${hours(value.estimatedWatchSeconds)} (${percentage(value.share)})`).join('<br>') || '—';
  ...
  return `<section class="section yt-topic-trend" data-topic-trend><div class="section-head"><div><h2>${t.topicTrendTitle}</h2><span>${t.topicTrendSub}</span></div>
    <div class="yt-trend-toggle" role="group" aria-label="${html(t.topicTrendView)}"><button type="button" data-trend-view="race" aria-pressed="true">${t.topicTrendRace}</button><button type="button" data-trend-view="heatmap" aria-pressed="false">${t.topicTrendHeatmap}</button></div></div>
    <p class="yt-trend-range" role="note">${t.topicTrendRangeNote}</p><p class="yt-trend-summary">${html(summary)}</p>
    <div data-trend-panel="race"><div class="yt-trend-race-controls"><button type="button" data-race-previous>${t.topicTrendPrevious}</button><button type="button" data-race-play data-play="${html(t.topicTrendPlay)}" data-pause="${html(t.topicTrendPause)}">${t.topicTrendPlay}</button><button type="button" data-race-next>${t.topicTrendNext}</button>
      <label>${t.topicTrendMonth}<input type="range" min="0" max="${latestIndex}" value="${latestIndex}" data-race-range></label><label>${t.topicTrendSpeed}<select data-race-speed><option value="1600">0.75×</option><option value="1000" selected>1×</option><option value="600">1.5×</option></select></label></div>
      ...
      <ol class="yt-trend-race" data-race-list aria-live="polite">${raceRows(model, latestIndex, t)}</ol></div>
    <div data-trend-panel="heatmap" hidden><div class="yt-trend-heat-controls"><label>${t.topicTrendSort}<select data-topic-sort><option value="latest">${t.topicTrendSortLatest}</option><option value="growth">${t.topicTrendSortGrowth}</option><option value="average">${t.topicTrendSortAverage}</option></select></label><span>${t.topicTrendSelectHelp}</span></div>
      <div class="yt-trend-heat-wrap"><div class="yt-trend-heat-head"><span>${t.topics}</span><div>${model.frames.map((frame) => `<span>${html(frame.label)}</span>`).join('')}</div><span>${t.topicTrendLatest}</span><span>${t.topicTrendChange}</span></div><div data-heat-rows>${heatmapRows(model, t)}</div></div>
    ...
    <script>(()=>{const root=document.currentScript?.closest('[data-topic-trend]');if(!root)return;const model=JSON.parse(root.querySelector('[data-topic-trend-data]').textContent);const text=${safeJson({
    ...
    })};const pct=(value)=>value===null?'—':(value*100).toFixed(1)+'%';const pp=(value)=>(value>0?'+':'')+(value*100).toFixed(1)+' pp';
      ...
      const range=root.querySelector('[data-race-range]');const list=root.querySelector('[data-race-list]');const month=root.querySelector('[data-race-month]');const coverage=root.querySelector('[data-race-coverage]');const play=root.querySelector('[data-race-play]');let timer;const stop=()=>{if(timer)clearInterval(timer);timer=undefined;play.textContent=text.play;play.setAttribute('aria-label',text.play)};const render=(index,manual=true)=>{if(manual)stop();range.value=String(index);const frame=model.frames[index];const prev=model.frames[index-1];month.textContent=frame.label;coverage.textContent=frame.coverage===null?text.noCoverage:text.coverage.replace('{n}',String(Math.round(frame.coverage*100)))+(frame.provisional?' · '+text.provisional:'');coverage.classList.toggle('is-provisional',frame.provisional);let values=frame.values.filter((value)=>value.share!==null).sort((a,b)=>b.share-a.share||a.name.localeCompare(b.name));const shown=values.slice(0,8);const rest=values.slice(8);if(rest.length)shown.push({slug:'__other',name:text.other,share:rest.reduce((sum,value)=>sum+value.share,0)});const max=Math.max(.01,...shown.map((value)=>value.share));list.replaceChildren(...shown.map((value,index)=>{const row=document.createElement('li');row.className='yt-trend-race-row';const old=value.slug==='__other'?null:prev?.values.find((candidate)=>candidate.slug===value.slug)?.share??null;const delta=old===null?null:value.share-old;const movement=delta===null?text.noChange:Math.abs(delta)<.0005?text.flat:pp(delta);row.innerHTML='<span class="yt-trend-rank"></span><span class="yt-trend-race-main"><strong></strong><span class="yt-trend-track"><i></i></span></span><span class="yt-trend-race-value"><strong></strong><small></small></span>';row.querySelector('.yt-trend-rank').textContent=String(index+1);row.querySelector('.yt-trend-race-main strong').textContent=value.name;row.querySelector('.yt-trend-track i').style.cssText='--bar:'+value.share/max*100+'%;--topic-color:'+(model.topics.find((topic)=>topic.slug===value.slug)?.color||'#77736d');row.querySelector('.yt-trend-race-value strong').textContent=pct(value.share);row.querySelector('small').textContent=movement;return row}))};
      root.querySelector('[data-race-previous]').addEventListener('click',()=>render(Math.max(0,Number(range.value)-1)));root.querySelector('[data-race-next]').addEventListener('click',()=>render(Math.min(model.frames.length-1,Number(range.value)+1)));range.addEventListener('input',()=>render(Number(range.value)));play.addEventListener('click',()=>{if(timer)return stop();if(Number(range.value)>=model.frames.length-1)render(0);play.textContent=text.pause;play.setAttribute('aria-label',text.pause);const tick=()=>{const next=Number(range.value)+1;if(next>=model.frames.length)return stop();render(next,false)};timer=setInterval(tick,Number(root.querySelector('[data-race-speed]').value))});
      const rows=[...root.querySelectorAll('[data-topic-row]')];const sort=root.querySelector('[data-topic-sort]');const sortRows=()=>{const key=sort.value;rows.sort((a,b)=>Number(b.dataset[key])-Number(a.dataset[key])||a.dataset.name.localeCompare(b.dataset.name));rows.forEach((row,index)=>row.style.order=String(index))};sort.addEventListener('change',sortRows);sortRows();const checks=[...root.querySelectorAll('[data-topic-select]')];checks.forEach((check)=>check.checked=${safeJson(selected)}.includes(check.value));const focus=root.querySelector('[data-topic-focus]');const svg=focus.querySelector('svg');const lines=svg.querySelector('.yt-trend-focus-lines');const ns='http://www.w3.org/2000/svg';const draw=()=>{const chosen=checks.filter((check)=>check.checked).map((check)=>check.value);checks.forEach((check)=>check.disabled=!check.checked&&chosen.length>=3);lines.replaceChildren();const max=Math.max(.05,...model.frames.flatMap((frame)=>frame.values.filter((value)=>chosen.includes(value.slug)&&value.share!==null).map((value)=>value.share)));const x=(index)=>20+index/Math.max(1,model.frames.length-1)*620;const y=(share)=>18+(1-share/max)*172;for(const [chosenIndex,slug] of chosen.entries()){const topic=model.topics.find((candidate)=>candidate.slug===slug);let path='';let open=false;for(let index=0;index<model.frames.length;index+=1){const share=model.frames[index].values.find((value)=>value.slug===slug)?.share??null;if(share===null){open=false;continue}path+=(open?'L':'M')+x(index).toFixed(1)+','+y(share).toFixed(1);open=true}const latest=[...model.frames].reverse().map((frame)=>frame.values.find((value)=>value.slug===slug)?.share??null).find((share)=>share!==null)??0;const labelY=Math.max(26,Math.min(190,y(latest)+(chosenIndex-(chosen.length-1)/2)*14));const group=document.createElementNS(ns,'g');group.style.setProperty('--topic-color',topic.color);const line=document.createElementNS(ns,'path');line.setAttribute('d',path);const label=document.createElementNS(ns,'text');label.setAttribute('x','650');label.setAttribute('y',String(labelY));label.textContent=topic.name+' '+pct(latest);group.append(line,label);lines.append(group)}svg.setAttribute('aria-label',text.compareAria)};checks.forEach((check)=>check.addEventListener('change',draw));draw();
      if(matchMedia('(prefers-reduced-motion:reduce)').matches)root.classList.add('reduce-motion')})();</script>
  </section>`;
}

export const topicTrendStyles = `
  .yt-topic-trend{overflow:hidden}.yt-trend-toggle{background:var(--raised);border:1px solid var(--line);border-radius:999px;display:flex;padding:2px}.yt-trend-toggle button{background:transparent;border:0;border-radius:999px;color:var(--muted);cursor:pointer;font:inherit;font-size:11px;font-weight:700;padding:6px 11px}.yt-trend-toggle button[aria-pressed=true]{background:var(--ink);color:#111}
  ...
  .yt-trend-race{display:grid;gap:9px;list-style:none;margin:0;padding:0}.yt-trend-race-row{align-items:center;display:grid;gap:10px;grid-template-columns:22px minmax(0,1fr) 94px}.yt-trend-rank{color:var(--muted);font-size:11px;text-align:right}.yt-trend-race-main{min-width:0}.yt-trend-race-main strong{display:block;font-size:12px;margin-bottom:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.yt-trend-track{background:var(--raised);border-radius:3px;display:block;height:12px;overflow:hidden}.yt-trend-track i{background:var(--topic-color);display:block;height:100%;width:var(--bar)}.yt-trend-race-value{text-align:right}.yt-trend-race-value strong,.yt-trend-race-value small{display:block;font-variant-numeric:tabular-nums}.yt-trend-race-value strong{font-size:12px}.yt-trend-race-value small{color:var(--muted);font-size:9px}
  .yt-trend-heat-controls{align-items:end;display:flex;justify-content:space-between;margin-bottom:12px}.yt-trend-heat-controls>span{color:var(--muted);font-size:10px}.yt-trend-heat-wrap{max-width:100%;overflow-x:auto;padding-bottom:6px}.yt-trend-heat-wrap [data-heat-rows]{display:flex;flex-direction:column}.yt-trend-heat-head,.yt-trend-heat-row{align-items:center;display:grid;gap:8px;grid-template-columns:150px minmax(600px,1fr) 52px 62px;min-width:900px}.yt-trend-heat-head{color:var(--muted);font-size:9px;padding:0 0 6px}.yt-trend-heat-head>div,.yt-trend-heat-cells{display:grid;gap:3px;grid-template-columns:repeat(12,minmax(47px,1fr));text-align:center}.yt-trend-heat-row{border-top:1px solid var(--line);padding:5px 0}.yt-trend-topic-label{align-items:center;display:flex;font-size:11px;gap:6px;min-width:0}.yt-trend-topic-label input{accent-color:var(--accent)}.yt-trend-topic-label i{background:var(--topic-color);border-radius:50%;height:7px;width:7px}.yt-trend-topic-label span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.yt-trend-heat-row>strong,.yt-trend-heat-row>span{font-size:10px;font-variant-numeric:tabular-nums;text-align:right}.yt-trend-heat-row>span{color:var(--muted)}.yt-trend-heat-cell{background:rgba(200,77,77,var(--heat));border:0;border-radius:4px;color:var(--ink);cursor:help;font:inherit;font-size:8px;height:28px;padding:0}.yt-trend-heat-cell.is-provisional{background-image:repeating-linear-gradient(135deg,transparent 0,transparent 4px,rgba(255,255,255,.2) 4px,rgba(255,255,255,.2) 6px)}.yt-trend-heat-cell:focus-visible{outline:2px solid var(--ink);outline-offset:1px}
  ...
  .yt-topic-trend-method{color:var(--muted);font-size:10px;line-height:1.5;margin:14px 0 0}.reduce-motion .yt-trend-track i{transition:none}
  ...
  @media(prefers-reduced-motion:reduce){.yt-trend-track i{transition:none}}
`;
