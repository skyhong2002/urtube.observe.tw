...
export function profileEditPage(user: User, csrf: string, lang: Lang, value: ProfileInput = user, error = '', saved = false): string {
  ...
  const body = `<style>${formStyles}
    ...
    </style><section class="ob-intro"><h1>${t.title}</h1><p>${t.privacy}</p>${user.googleSub && !user.avatarUrl ? `<p>${lang === 'zh' ? '尚未取得 Google 頭貼，暫以 Gravatar 或字首圖顯示。' : 'No Google picture has been saved yet; using Gravatar or initials for now.'} <a href="/auth/google?next=%2Faccount%2Fprofile">${lang === 'zh' ? '重新登入以取得 Google 頭貼' : 'Sign in again to get your Google picture'}</a></p>` : ''}</section>
