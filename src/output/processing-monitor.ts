  ...
  function render(data) {
    ...
    if (data.pipeline) {
      ...
      for (const stage of data.pipeline) {
        ...
        if (stage.done !== null && stage.total !== null) section.append(node('span', number(stage.done) + ' / ' + number(stage.total) + (stage.id === 'embedding' ? ' tags' : t(' 部影片', ' videos'))));
        const eta = stage.state === 'done' ? t('剩餘 0 分鐘', '0 min remaining')
          : stage.estimatedMinutes !== null ? t('估計約 ', 'About ') + number(stage.estimatedMinutes) + t(' 分鐘', ' min')
          : stage.state === 'running' ? t('時間估算中', 'Estimating time')
          : t('時間待估', 'ETA pending');
        const time = node('span', eta); time.className = 'yt-pipeline-eta'; section.append(time);
    ...
    const diagnostics = node('details'); diagnostics.className = 'yt-monitor-details'; diagnostics.open = detailsOpen;
    diagnostics.addEventListener('toggle', () => { detailsOpen = diagnostics.open; });
    diagnostics.append(node('summary', t('處理明細與說明', 'Processing details')));
    output.append(diagnostics); fragment = diagnostics;
    fragment.append(...explanations);
    fragment.append(node('p', t('時間依近期速度估算，不含排隊與重試。', 'Time estimates use recent progress and exclude queueing and retries.')));
    if (data.pipeline) fragment.append(node('h3', t('工作明細', 'Job details')));
    ...
    if (profile) {
      ...
      details.append(node('h3', t('各類影片', 'Videos by category')));
      const list = node('dl'); list.className = 'yt-monitor-genres';
      for (const genre of data.genres) {
        const result = profile.genres[genre];
        list.append(node('dt', zh ? (genres[genre] || genre) : genre), node('dd', (result ? number(result.videoCount) + t(' 部已分類影片', ' classified videos') : t('尚未建立', 'Not built'))));
      }
