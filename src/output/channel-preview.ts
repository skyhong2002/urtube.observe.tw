...
export function channelPreview(data: ChannelPageData, lang: Lang): string {
  ...
  if (data.community) {
    ...
    if (data.community.members.length) community += section(t.channelTopViewers, `<ol class="cp-members">${order(data.community.members).slice(0, 5).map((member, i) => `<li><span>#${i + 1}</span><img src="/avatar/member/${html(member.handle)}" alt="" width="32" height="32" loading="lazy"><strong>${html(member.displayName)}</strong><small>${hours(member.estimatedWatchSeconds)} · ${html(t.matchesTimes(member.watches))}</small></li>`).join('')}</ol>`);
  }
  const dates = [
...
const previewStyles = `
  ...
  .cp-members li{align-items:center;display:flex;gap:9px;font-size:12px}.cp-members li>span{color:var(--muted);font-size:10px;min-width:18px}.cp-members img{border-radius:50%;height:32px;width:32px}.cp-members strong{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cp-members small{flex:none}.cp-dates{color:var(--muted);display:flex;flex-wrap:wrap;font-size:11px;gap:12px 24px}.cp-dates dd{color:var(--ink-2);margin:3px 0}
