...
export function v3DashboardSection(profile: Profile | null, options: {
  ...
  return `<section class="section yt-v3-interests" data-v3-interests${options.ownerDetails ? ' data-tag-clouds' : ''}${!current ? ' data-processing-status' : ''}>
    ...
    <p class="yt-v3-scope">${html(scope)}</p>${options.ownerDetails ? `<p class="yt-v3-scope">${zh ? '文字大小依不同影片中的標籤次數呈現；使用已建立興趣群的原始代表標籤，不是完整觀看紀錄。此標籤雲僅自己可見。' : 'Word sizes reflect distinct-video tag counts in existing interest clusters, not your full watch history. Tag clouds are visible only to you.'}</p>` : ''}
    ${current ? `<div class="yt-v3-grid">${visible.map(genre => {
      const item = current.genres[genre];
      return `<div class="yt-v3-genre"><strong>${html(zh ? names[genre] : genre)}</strong><span>${html(state(genre))}</span>${cloud(genre)}<details><summary>${zh ? '分析狀態' : 'Analysis status'}</summary>${item?.clusters.length && (item.status === 'insufficient' || !current.complete || options.provisional) ? `<span>${zh ? '暫定分析' : 'Provisional analysis'}</span>` : ''}${item?.clusters.length && item.retainedCoverage < 0.5 ? `<span>${zh ? '核心興趣涵蓋較少，尚不能代表整體' : 'Core interests cover a limited portion of this category'}</span>` : ''}${item ? `<span>${Number(item.videoCount).toLocaleString('en')} ${zh ? '部影片' : 'videos'}</span>` : ''}</details></div>`;
