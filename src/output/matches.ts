...
function inboxSection(inbox: MatchingInbox, lang: Lang): string {
  const t = messages(lang);
  const incoming = inbox.incoming.length ? `<section class="mt-inbox-section"><h2>${t.matchesIncoming}</h2><div class="mt-inbox-list">${inbox.incoming.map((request) => `<div class="mt-inbox-row"><div class="mt-inbox-copy"><strong>${html(request.displayName)}</strong><span>${html(request.topics.join(lang === 'zh' ? '、' : ', ') || t.matchesSharedGroundPrivate)}</span></div><div class="mt-inbox-actions"><form method="post" action="/matches/respond"><input type="hidden" name="requestToken" value="${html(request.requestToken)}"><button class="mt-accept" name="response" value="accept">${t.matchesAccept}</button><button class="mt-secondary" name="response" value="decline">${t.matchesDecline}</button></form><button class="mt-secondary mt-ignore" type="button">${t.matchesSkip}</button></div></div>`).join('')}</div></section>` : '';
  const sent = inbox.sent.length ? `<section class="mt-inbox-section"><h2>${t.matchesSent}</h2><div class="mt-inbox-list">${inbox.sent.map((request) => `<div class="mt-inbox-row"><div class="mt-inbox-copy"><strong>${html(request.displayName)}</strong><span>${t.matchesAwaiting}</span></div><form method="post" action="/matches/withdraw"><input type="hidden" name="requestToken" value="${html(request.requestToken)}"><button class="mt-secondary" type="submit">${t.matchesWithdraw}</button></form></div>`).join('')}</div></section>` : '';
  const connections = inbox.connections.length ? `<section class="mt-inbox-section"><h2>${t.matchesConnections}</h2><div class="mt-connections">${inbox.connections.map((connection) => `<article class="mt-connection"><h3>${html(connection.displayName)}</h3>${connection.introduction ? `<p>${html(connection.introduction)}</p>` : `<p>${t.matchesNoIntroduction}</p>`}<p>${html(icebreaker(connection.topics, lang))}</p>${connection.contact ? `<p class="mt-contact">${html(connection.contact)}</p>` : `<p class="mt-contact">${t.matchesNoContact}</p>`}</article>`).join('')}</div></section>` : '';
