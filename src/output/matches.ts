...
export interface ActionableMatchingCandidateCard extends MatchingCandidateCard {
  ...
  topicMatch?: { score: number | null; provisional: boolean; reasons: string[]; detailsVisible: boolean; keywords?: string[]; details?: Array<{genre: Genre; score: number | null}>; available?: Genre[]; selected?: Genre[]; unavailable?: 'pending' | 'consent' | 'service' };
}
...
const matchesStyles = `
  ...
  .mt-card[data-compact-friendship] .mt-person{padding-right:44px}.mt-card[data-compact-friendship="incoming"] .mt-person{padding-right:92px}.mt-card .mt-friend-tools{position:absolute;right:12px;top:12px;margin:0}.mt-card .mt-friend-tools form{flex-wrap:wrap;justify-content:flex-end;gap:8px}.mt-card .mt-friend-tools button{width:40px;height:40px;min-height:40px;padding:0;gap:0;border:1px solid var(--line-strong);border-radius:50%;background:var(--raised);color:var(--ink);font-size:13px;font-weight:700}.mt-card[data-compact-friendship="none"] .mt-friend-tools button,.mt-card .mt-friend-tools button[value="accept"]{background:var(--accent);border-color:var(--accent);color:#fff}.mt-card[data-compact-friendship="sent"] .mt-friend-tools button{background:rgba(250,178,25,.15);border-color:#9d7626;color:#f5c95e}.mt-card[data-compact-friendship="connected"] .mt-friend-tools button{background:rgba(78,190,130,.15);border-color:#3d825f;color:#71d9a1}.mt-card .mt-friend-tools button:disabled{opacity:.6;cursor:wait}.mt-friend-feedback{padding:10px 14px;border:1px solid var(--line-strong);border-radius:8px;font-size:13px;color:var(--ink);background:var(--raised)}
  ...
  .mt-blend-keywords{display:flex;flex-wrap:wrap;justify-content:center;gap:6px 10px;padding:10px 4px;margin-top:8px;background:var(--bg);color:var(--muted);font-size:12px;line-height:1.5;text-align:center;max-width:100%;overflow-wrap:anywhere}.mt-blend-keywords span{max-width:100%}
  @media(max-width:620px){.mt-blend-keywords{font-size:10px;gap:5px;padding:8px 2px}.mt-vs{grid-template-columns:minmax(0,1fr) 66px minmax(0,1fr)}.mt-vs-score{border-width:4px;height:62px;width:62px}.mt-vs-score strong{font-size:16px}.mt-vs-score span{font-size:7px}.mt-side{min-height:300px;padding:20px 8px}.mt-side h2{overflow-wrap:anywhere}.mt-side .mt-pill{font-size:9px;padding:3px 6px}}
`;
...
function friendshipIcons(card: ActionableMatchingCandidateCard, t: Messages): string {
  ...
  const icon = (kind: 'add' | 'connected' | 'pending' | 'accept' | 'decline') => `<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${kind === 'pending' ? '<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>' : kind === 'connected' ? '<circle cx="8" cy="7" r="3"/><path d="M2 20v-2a6 6 0 0 1 12 0v2m2-9 2 2 4-5"/>' : kind === 'accept' ? '<path d="m5 12 4 4L19 6"/>' : kind === 'decline' ? '<path d="m6 6 12 12M6 18 18 6"/>' : `<circle cx="9" cy="7" r="3"/><path d="M3 20v-2a6 6 0 0 1 12 0v2M17 9h6"/>${kind === 'add' ? '<path d="M20 6v6"/>' : ''}`}</svg>`;
  const button = (label: string, kind: 'add' | 'connected' | 'pending' | 'accept' | 'decline', value?: string) => `<button type="submit" aria-label="${html(label)}" title="${html(label)}"${value ? ` name="response" value="${value}"` : ''}>${icon(kind)}</button>`;
  ...
  const controls = relationship.status === 'none' ? button(t.matchesAddFriend, 'add')
    : relationship.status === 'incoming' ? button(t.matchesAcceptFriend, 'accept', 'accept') + button(t.matchesDecline, 'decline', 'decline')
      : relationship.status === 'sent' ? button(`${t.matchesFriendSent} · ${t.matchesWithdraw}`, 'pending')
        : button(t.matchesDisconnect, 'connected');
...
export function matchingCandidatePage(
  ...
  const keywords = card.topicMatch?.detailsVisible ? card.topicMatch.keywords ?? [] : [];
  const keywordHtml = keywords.length ? `<div class="mt-blend-keywords" aria-label="${lang === 'zh' ? '共同興趣關鍵字' : 'Shared interest keywords'}">${keywords.map(word => `<span>${html(word)}</span>`).join('')}</div>` : '';
  ...
  const header = `<div class="mt-vs"><section class="mt-side"><span class="mt-side-label">${t.matchesYou}</span><a class="mt-profile-link" href="/${html(viewer.handle)}?range=${comparison.range}&lang=${lang}"><img src="/avatar/member/${html(viewer.handle)}" alt="" width="116" height="116"><h2>${html(viewerName)}</h2></a>${viewerInterests}</section><div class="mt-vs-center"><div class="mt-vs-score"><strong>${score}</strong><span>${t.matchesFit}</span></div>${keywordHtml}${missingNote ? `<p class="mt-gate">${html(missingNote)}</p>` : ''}</div><section class="mt-side"><span class="mt-side-label">${t.matchesCandidate}</span><a class="mt-profile-link" href="/${html(card.handle)}?range=${comparison.range}&lang=${lang}"><img src="/avatar/member/${html(card.handle)}" alt="" width="116" height="116"><h2>${html(card.displayName)}</h2></a>${candidateInterests}</section></div>`;
