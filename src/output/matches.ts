...
const matchesStyles = `
  ...
  .mt-card[data-compact-friendship] .mt-person{padding-right:0}.mt-card .mt-friend-tools{position:static;order:-1;align-self:flex-end;max-width:100%;margin:0 0 12px}.mt-card .mt-friend-tools form{flex-wrap:wrap;justify-content:flex-end;gap:8px}.mt-card .mt-friend-tools button{width:auto;height:auto;min-height:44px;padding:10px 14px;gap:7px;border:1px solid var(--line-strong);border-radius:999px;background:var(--raised);color:var(--ink);font-size:13px;font-weight:700}.mt-card[data-compact-friendship="none"] .mt-friend-tools button,.mt-card .mt-friend-tools button[value="accept"]{background:var(--accent);border-color:var(--accent);color:#fff}.mt-card .mt-friend-tools button:disabled{opacity:.6;cursor:wait}.mt-friend-feedback{padding:10px 14px;border:1px solid var(--line-strong);border-radius:8px;font-size:13px;color:var(--ink);background:var(--raised)}
...
function friendshipIcons(card: ActionableMatchingCandidateCard, t: Messages): string {
  ...
  const button = (label: string, kind: 'add' | 'remove' | 'accept' | 'decline', value?: string) => `<button type="submit" aria-label="${html(label)}" title="${html(label)}"${value ? ` name="response" value="${value}"` : ''}>${icon(kind)}<span>${html(label)}</span></button>`;
