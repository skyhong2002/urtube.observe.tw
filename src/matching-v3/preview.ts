...
export async function publishCachedPreviews(registry: UserRegistry, s: Settings, compute: Compute) {
  ...
  for (const user of registry.listUsers()) {
    if (store.profile(user.id)?.version === version(s)) { skipped++; continue; }
    ...
    if (!Object.values(profile.genres).some(g => g.clusters.length)
      || registry.repositoryFor(user).matchingV3Source(s.backfillVideoLimit).fingerprint !== source.fingerprint) { skipped++; continue; }
    if (store.publishPreview(user.id, profile)) published++; else skipped++;
  }
